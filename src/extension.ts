...
// Fix for TypeScript in environment without @types/node
                ...
                if (!rawOutput.includes("ymm0")) {
                    ...
                    results.forEach(res => {
                        if (res.val) {
                            ...
                            if (cleanVal.includes("=")) cleanVal = cleanVal.substring(cleanVal.indexOf("=") + 1).trim();
                            extraData += `\nymm${res.id} ${cleanVal}`;
                        }
                    });
                    if (extraData) rawOutput += "\n--- FORCED YMM FETCH ---" + extraData;
                }
...
function getWebviewContent(defaultPlatform: string) {
    return `
    <!DOCTYPE html>
    <html>
    <head>
